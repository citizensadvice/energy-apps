# frozen_string_literal: true

module CsrTable
  class CommunicationScoresComponent < ViewComponent::Base
    attr_reader :supplier, :renderer

    def initialize(supplier)
      super()
      @supplier = supplier
      @renderer = Renderers::RichTextRenderer.new
    end

    def render?
      supplier.present?
    end

    def descriptions
      descriptions = [
        contact_time,
        contact_email
      ]
      check_for_additional_descriptions(descriptions)
    end

    def check_for_additional_descriptions(descriptions)
      # Not all suppliers will provide data for each of the social media channels;
      # and if a supplier provides synchronous data then they won't provide
      # asynchronous data. We therefore need to determine which fields to render
      # based on whether there is a value present or not
      descriptions << webchat_sync if supplier.contact_webchat_sync.present?
      descriptions << whatsapp_sync if supplier.contact_whatsapp_sync.present?
      descriptions << sms_sync if supplier.contact_sms_sync.present?
      descriptions << in_app_sync if supplier.contact_in_app_sync.present?
      descriptions << portal_sync if supplier.contact_portal_sync.present?
      descriptions << webchat_async if supplier.contact_webchat_async
      descriptions << whatsapp_async if supplier.contact_whatsapp_async
      descriptions << sms_async if supplier.contact_sms_async
      descriptions << in_app_async if supplier.contact_in_app_async
      descriptions << portal_async if supplier.contact_portal_async
      descriptions
    end

    def webchat_sync
      {
        term: content_tag(:p, "Webchat sync (hours, minutes and seconds)"),
        description: content_tag(:p, supplier.contact_webchat_sync)
      }
    end

    def webchat_async
      {
        term: content_tag(:p, "Webchat (async)"),
        description: content_tag(:p, "#{supplier.contact_webchat_async}%")
      }
    end

    def whatsapp_sync
      {
        term: content_tag(:p, "Whatsapp sync (hours, minutes and seconds)"),
        description: content_tag(:p, supplier.contact_whatsapp_sync)
      }
    end

    def whatsapp_async
      {
        term: content_tag(:p, "Whatsapp (async)"),
        description: content_tag(:p, "#{supplier.contact_whatsapp_async}%")
      }
    end

    def sms_sync
      {
        term: content_tag(:p, "SMS sync (hours, minutes and seconds)"),
        description: content_tag(:p, supplier.contact_sms_sync)
      }
    end

    def sms_async
      {
        term: content_tag(:p, "SMS (async)"),
        description: content_tag(:p, "#{supplier.contact_sms_async}%")
      }
    end

    def in_app_sync
      {
        term: content_tag(:p, "In App sync (hours, minutes and seconds)"),
        description: content_tag(:p, supplier.contact_in_app_sync)
      }
    end

    def in_app_async
      {
        term: content_tag(:p, "In App (async)"),
        description: content_tag(:p, "#{supplier.contact_in_app_async}%")
      }
    end

    def portal_sync
      {
        term: content_tag(:p, "Portal sync (hours, minutes and seconds)"),
        description: content_tag(:p, supplier.contact_portal_sync)
      }
    end

    def portal_async
      {
        term: content_tag(:p, "Portal (async)"),
        description: content_tag(:p, "#{supplier.contact_portal_async}%")
      }
    end

    def contact_time
      {
        term: content_tag(:p, "Average call centre wait time (hours, minutes and seconds)"),
        description: content_tag(:p, supplier.contact_time)
      }
    end

    def contact_email
      {
        term: content_tag(:p, "Emails responded to within 2 days"),
        description: content_tag(:p, "#{supplier.contact_email}%")
      }
    end
  end
end
